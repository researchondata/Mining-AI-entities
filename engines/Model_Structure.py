...
class Model_Structure(object):
    ...
    def build(self):
        ...
        if not self.is_crf:
            ...
            self.w = tf.cond(self.epoch[0]<45, lambda:1/(1+tf.exp(alpha*(-self.log_likelihood-tau))), lambda:tf.cond(self.epoch[0]>75 , lambda:1/(1+tf.exp(-alpha*(-self.log_likelihood-tau))), lambda:1))
