...
class Configer:
    def __init__(self, config_file='system.config'):
        ...
        the_item = 'pos_emb_dim'
        if the_item in config:
            self.pos_emb_dim = int(config[the_item])

        the_item = 'use_pos_emb'
        if the_item in config:
            self.use_pos_emb = self.str2bool(config[the_item])
        ...
        the_item = 'output_excel'
        if the_item in config:
            self.output_excel = config[the_item]
        the_item = 'raw_real_sens_file'
        if the_item in config:
            self.raw_real_sens_file = config[the_item]
        the_item = 'output_methods_excel'
        if the_item in config:
            self.output_methods_excel = config[the_item]
        the_item = 'output_experiment_excel'
        if the_item in config:
            self.output_experiment_excel = config[the_item]
        the_item = 'output_paper_unit_excel'
        if the_item in config:
            self.output_paper_unit_excel = config[the_item]
        the_item = 'output_normalized_excel'
        if the_item in config:
            self.output_normalized_excel = config[the_item]
    ...
    def show_data_summary(self, logger):
        ...
        logger.info(" " + "++" * 20)
        logger.info(" Training Settings:")
        logger.info("     raw_real_sens_file  : %s" % (self.raw_real_sens_file))
        logger.info("     output_excel        : %s" % (self.output_excel))
        logger.info(" " + "++" * 20)
